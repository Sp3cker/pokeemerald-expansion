...
#define FRIENDSHIP_EVO_THRESHOLD ((P_FRIENDSHIP_EVO_THRESHOLD >= GEN_8) ? 160 : 220)
 u16 gOriginalSpecies = SPECIES_NONE; // Global variable to store the original species before randomization
struct SpeciesItem
{
    u16 species;
    u16 item;
...
void PokemonToBattleMon(struct Pokemon *src, struct BattlePokemon *dst)
{
    ...
    dst->originalSpecies = dst->species; // Default to current species (for non-randomized mons)
